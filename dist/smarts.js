  ...
  var smarts = {
    ...
    createScopedEval(uuid) {
      ...
      `
				function createScopedEval(${uuid}){
					...
					// scopeCode
					${uuid}.scopeCode = ${uuid}.scopeCode || ${uuid}.smarts.babel.template.ast('try{}catch(){}')
					${uuid}.previousScopeCode = ${uuid}.currentScopeCode || ${uuid}.scopeCode
					${uuid}.currentScopeCode = ${uuid}.scopeCode.block.body.length ? ${uuid}.smarts.babel.template.ast('try{}catch(){}') : ${uuid}.scopeCode
					${uuid}.closureIndex = ${uuid}.closureIndex || 0
					${uuid}.closure = (${uuid}.val.$scopes && ${uuid}.val.$scopes[${uuid}.closureIndex]) || {}
					${uuid}.variableKeys = Object.keys(${uuid}.closure)
					${uuid}.variableMap = (${uuid}.val.$state.$variableMaps && ${uuid}.val.$state.$variableMaps[${uuid}.closureIndex]) || []
					// console.log(${uuid}.val.$state)
					// console.log(${uuid})
					// console.log(${uuid}.closureIndex)
					// console.log((${uuid}.val.$scopes && ${uuid}.val.$scopes[${uuid}.closureIndex]) || {})
					// console.log(${uuid}.val.$scopes.length)
					// console.log(${uuid}.closureIndex >= ${uuid}.val.$scopes.length)
					${uuid}.variableKeys.forEach((key)=>{
						if(typeof ${uuid}.variableMap[key] == 'string'){
							${uuid}.currentScopeCode.block.body.append(
      ...
      `\`
					...
					// console.log(${uuid}.scopeCode)
					${uuid}.closureIndex++
					if(${uuid}.closureIndex >= ${uuid}.val.$scopes.length){
						// console.log(${uuid}.scopeCode)
						let ret = eval(${
      /*javascript*/
      `
      ...
      `\`(\${${uuid}.smarts.createScopedEval('${uuid}')})\``})(${uuid})
						// let ret = function(){return function(){}}
						return ret
					}
				}
			`;
    ...
    parser(opts) {
      return function (key, val) {
        if (val.$js && opts.replaceMode) {
          ...
          if (val.$loader && typeof val.$loader == 'function') {
            loader = val.$loader;
          } else {
				...
				(
					function(){
						let ${uuid} = {
							...
							$scopes: [...(typeof $state != 'undefined') ? $state.$scopes : []],
							$variableMaps: [...(typeof $state != 'undefined') ? $state.$variableMaps : []],
							uuid: '${uuid}'
						}
    ...
    initState(uuid, aster) {
      ...
      `
				...
				var $state = ${uuid}
			`);
      node[0].declarations[0].init.properties[2].value.scoperWrapped = true;
      node[0].declarations[0].init.properties[2].value.body.scopeInitialized = true;
      return node;
    },
    ...
    initBlock(path, aster) {
      if (!path.node.scopeInitialized) {
        ...
        let uuid = smarts.jsUUID();
        let i = smarts.bodyInsert(0, path.node.body, aster, ...smarts.initState(uuid, aster));
        ...
        visitor: {
          ...
          VariableDeclaration(path) {
            if (!path.node.inScope) {
              ...
              let uuid = path.contexts[0].scope.uuid;
              path.insertAfter(smarts.scopeVar(uuid, path.node.declarations[0].id.name, path.node.kind, aster));
            }
          }
