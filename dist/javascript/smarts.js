...
module.exports = function () {
  ...
  var smarts = {
    ...
    toJavascript(value) {
      ...
      opts = defaultOpts;
      smarts.toJavascriptAux(value, opts); // sort declarations by dependancies

      for (let declaration of opts.declarations) {
        if (declaration.type === "ExpressionStatement") {} else {
          ...
          for (let sortableDeclaration of opts.declarations) {
            if (sortableDeclaration.type === "ExpressionStatement") {} else {
      ...
      if (opts.wrapInFunction && !opts.moduleExport) {
        ...
        let expression = t.expressionStatement(t.callExpression(t.functionExpression(null, [], t.blockStatement(opts.declarations)), []));
        program = t.program([expression]);
      } else {
        if (opts.moduleExport) {
          opts.declarations.push(t.expressionStatement(t.assignmentExpression("=", t.memberExpression(t.identifier("module"), t.identifier("exports")), t.identifier("obj"))));
        }
      ...
      return smarts.getBabel().prettier.format(stringifiedProgram, {
        semi: false,
        parser: "babel",
        useTabs: true
      });
    ...
    toJavascriptAux(value, opts) {
      if (typeof value === "object") {
        if (!opts.seen.includes(value)) {
          ...
          let declaration = t.variableDeclaration("let", [t.variableDeclarator(t.identifier(opts.identifier), t.objectExpression(properties))]);
          ...
          for (let key of Object.keys(value)) {
            ...
            if (property) {
              ...
              smarts.toJavascriptAux(value[key], { ...opts,
                identifier
              });
            }
          }
        ...
        let declaration = t.variableDeclaration("let", [t.variableDeclarator(t.identifier(opts.identifier), t.stringLiteral(value))]);
        opts.declarations.unshift(declaration);
      }
    },
    ...
    createObjectProperties(value, opts) {
      ...
      for (let key of Object.keys(value)) {
        if (value[key] === value) {
          opts.declarations.push(t.expressionStatement(t.assignmentExpression("=", t.memberExpression(t.identifier(opts.identifier), t.identifier(key)), t.identifier(opts.identifier))));
        } else {
    ...
    stringify(value) {
      ...
      for (let i = parseInt(smarts.setKnown(opts.known, opts.input, opts.virtual)); i < opts.input.length; i++) {
        opts.firstRun = true;
    ...
    stringifier(key, val, opts) {
      ...
      if (val instanceof Function && typeof val.toString === 'function' && (!opts.strictFunctions || typeof val.$scopes != 'undefined')) {
        ...
        smarts.setKnown(opts.known, opts.input, ret);
      }
    ...
    jsUUID() {
      let prefix = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'uuid';
    ...
    bodyInsert(index, body, aster) {
      for (var _len = arguments.length, things = new Array(_len > 3 ? _len - 3 : 0), _key = 3; _key < _len; _key++) {
        things[_key - 3] = arguments[_key];
      }
    ...
    schema(obj1, obj2) {
      ...
      return smarts.merge(obj1, obj2, { ...opts
      });
    },
    ...
    merge(value1, value2) {
      ...
      if (smarts.basic(value1) && !opts.overwrite) {
        return value1;
      }

      if (opts.overwrite && smarts.basic(value2)) {
        return value2;
      }
      ...
      props.forEach(prop => {
        ...
        if (prop in value1) {
          newVal = smarts.merge.bind(this)(propertyValue1, propertyValue2, { ...opts,
            ...{
              clone: false
            }
          }, seen);
    ...
    // },
    deepForEach(value, fn, path) {
      let ret = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
      let seens = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {
        originals: [],
        clones: []
      };
      let first = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : true;
      path = path || '';
    ...
    forEachArray(array, fn, path) {
      let ret = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
      let seens = arguments.length > 4 ? arguments[4] : undefined;
    ...
    optIn(option) {
      let list = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : smarts.getsmart.bind(this)(stringList);
      let obj = arguments.length > 2 ? arguments[2] : undefined;
      let keys = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : ['uuid', '_id', 'id'];
      let keymatchtype = arguments.length > 4 ? arguments[4] : undefined;
      let index = arguments.length > 5 ? arguments[5] : undefined;
    ...
    // no use right now
    ratchetOpt(option, list, obj) {// find(obj, property, equals){
    ...
    gosmarter(obj, property, value, context) {
      let schema = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : true;
    ...
    flatten(arrays) {
      let func = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : i => i;
      const flat = [];
    ...
    mapsmart(list) {
      let keyProperty = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'title';
      let returnExistant = arguments.length > 2 ? arguments[2] : undefined;
      let populate = arguments.length > 3 ? arguments[3] : undefined;
    ...
    getParent() {
      let levels = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : Infinity;
    ...
    equal(obj1, obj2) {
      let seen = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
